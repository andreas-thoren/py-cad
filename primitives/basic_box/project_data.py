...
class BoxDimensionData(DimensionData):
    def __init__(
        ...
        self.offset_x = material_thickness[PartType.SHORT_SIDE_PANEL] - route_depth
        self.offset_y = material_thickness[PartType.LONG_SIDE_PANEL] - route_depth
        self.offset_z = material_thickness[PartType.TOP] - route_depth
    ...
    def get_part_types_dimensions(self) -> dict[PartType, BasicDimensionData]:
        panel_y = self.y_len - 2 * self.offset_y
        panel_z = self.z_len - self.offset_z

        btm_x = self.x_len - 2 * self.offset_x
        ...
        return {
            ...
            PartType.LONG_SIDE_PANEL: BasicDimensionData(
                self.x_len, long_panel_thickness, panel_z
            ),
            PartType.SHORT_SIDE_PANEL: BasicDimensionData(
                short_panel_thickness, panel_y, panel_z
            ),
